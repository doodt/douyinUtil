...
import com.jinmgr.utils.DouyinUtil;
...
//                                    outputFile = downloadM3u8(module.getTitle().trim(), module.getM3u8Url().trim(), task.getDownloadDir() + FILESEPARATOR + task.getTaskName());
    ...
    private File mergeMedia(DownTask task) {
        ...
        try {
            ...
            //获取mp4本地文件地址并集下标排序(创建时间排序不可靠)
            List<String> fileListPath = task.getMediaModules().stream().sorted(Comparator.comparing(t -> t.getPlayIndex())).map(f -> f.getMediaLocalPath()).collect(Collectors.toList());
            //合并
